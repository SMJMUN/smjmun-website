  ...
  return (
    <section>
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <h3 className="font-sans text-[10px] font-600 tracking-[0.22em] uppercase text-white/50">
          {title}
        </h3>
      </div>
      {children}
...
function SidebarPostItem({ post, index }: { post: Blog; index?: number }) {
  ...
  return (
    <Link
      ...
      className="group flex gap-3 items-start no-underline py-3.5 border-b border-white/10 last:border-0"
    >
      {/* Thumbnail or number */}
      {typeof index === "number" ? (
        <span
        ...
        <div className="flex-shrink-0 w-[56px] h-[40px] bg-[#1A1A1A] overflow-hidden relative">
          <Image
            src={coverUrl}
            alt={post.title}
            fill
        ...
        <div className="flex-shrink-0 w-[56px] h-[40px] bg-white/10" />
      )}
      ...
      <div className="flex-1 min-w-0">
        ...
        <h4 className="font-serif text-[13px] font-bold leading-[1.3] text-white line-clamp-2 group-hover:text-white/80 transition-colors duration-300">
          {post.title}
        </h4>
