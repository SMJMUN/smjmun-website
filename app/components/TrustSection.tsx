'use client';

import { motion } from 'framer-motion';
import { ChevronLeft, ChevronRight } from 'lucide-react';
import { useEffect, useRef } from 'react';

const partners = [
  {
    name: 'Delhi Public School',
    logo: 'data:image/png;base64,',
  },
  {
    name: 'The Heritage School',
    logo: 'data:image/png;base64,',
  },
  {
    name: 'Amity University',
    logo: 'data:image/png;base64,',
  },
  {
    name: 'Oakridge International',
    logo: 'data:image/png;base64,',
  },
  {
    name: 'JBCN',
    logo: 'data:image/png;base64,',
  },
  {
    name: 'Pathways',
    logo: 'data:image/png;base64,',
  },
  {
    name: 'Ryan International',
    logo: 'data:image/png;base64,',
  },
  {
    name: 'Sanskriti School',
    logo: 'data:image/png;base64,',
  },
  {
    name: 'Modern School',
    logo: 'data:image/png;base64,',
  },
  {
    name: 'Shiv Nadar School',
    logo: 'data:image/png;base64,',
  },
];

export default function TrustSection() {
  const scrollRef = useRef<HTMLDivElement>(null);

  const scrollLeft = () => {
    scrollRef.current?.scrollBy({ left: -400, behavior: 'smooth' });
  };

  const scrollRight = () => {
    scrollRef.current?.scrollBy({ left: 400, behavior: 'smooth' });
  };

  useEffect(() => {
    const slider = scrollRef.current;
    if (!slider) return;

    const interval = setInterval(() => {
      slider.scrollBy({ left: 250, behavior: 'smooth' });

      if (slider.scrollLeft + slider.clientWidth >= slider.scrollWidth - 100) {
        slider.scrollTo({ left: 0, behavior: 'smooth' });
      }
    }, 1000);

    return () => clearInterval(interval);
  }, []);

  return (
    <section
      id="partnerships"
      className="relative overflow-hidden bg-[#0A0A0A] py-24"
    >
      {/* Floral Left */}
      <img
        src="/images/floral-left.svg"
        alt=""
        className="pointer-events-none absolute left-0 top-0 w-[110px] md:w-[180px] lg:w-[220px] opacity-[0.07]"
      />

      {/* Floral Right */}
      <img
        src="/images/floral-right.svg"
        alt=""
        className="pointer-events-none absolute right-0 top-0 w-[110px] md:w-[180px] lg:w-[220px] opacity-[0.07]"
      />

      <div className="relative z-10 mx-auto max-w-[1500px] px-8 lg:px-20">

        {/* Label */}
        <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ delay: 0.3 }}
          className="mb-14 text-center"
        >
          <span className="section-label">Trusted By Leading Institutions</span>
        </motion.div>

        <div className="relative mt-10">
          <div className="flex items-center gap-8">
            {/* Left Arrow */}
            <button
              onClick={scrollLeft}
              className="
                z-30 flex h-12 w-12 shrink-0 items-center justify-center
                rounded-full border border-[rgba(255,255,255,0.08)]
                bg-[#171717] text-[#BB8B57]
                transition-all duration-300
                hover:border-[#BB8B57] hover:bg-[rgba(187,139,87,0.08)]
                hidden md:flex
              "
            >
              <ChevronLeft size={18} />
            </button>

            {/* Logos */}
            <div
              ref={scrollRef}
              className="flex-1 overflow-x-auto scrollbar-hide scroll-smooth"
            >
              <div className="flex min-w-max items-center">
                {partners.map((partner, index) => (
                  <div key={partner.name} className="flex shrink-0 items-center">
                    <div className="flex items-center gap-4 px-8">
                      <img
                        src={partner.logo}
                        alt={partner.name}
                        className="object-contain opacity-70"
                        style={{
                          filter: 'brightness(0) invert(1)',
                          height: '32px',
                          width: 'auto',
                        }}
                      />
                      <span
                        className="text-[13px] font-medium uppercase tracking-[0.06em] text-[#B8B8B8] whitespace-nowrap"
                        style={{ fontFamily: 'var(--font-body), system-ui, sans-serif' }}
                      >
                        {partner.name}
                      </span>
                    </div>

                    {index !== partners.length - 1 && (
                      <div className="h-10 w-px bg-[rgba(255,255,255,0.08)]" />
                    )}
                  </div>
                ))}
              </div>
            </div>

            {/* Right Arrow */}
            <button
              onClick={scrollRight}
              className="
                z-30 flex h-12 w-12 shrink-0 items-center justify-center
                rounded-full border border-[rgba(255,255,255,0.08)]
                bg-[#171717] text-[#BB8B57]
                transition-all duration-300
                hover:border-[#BB8B57] hover:bg-[rgba(187,139,87,0.08)]
                hidden md:flex
              "
            >
              <ChevronRight size={18} />
            </button>
          </div>
        </div>
      </div>
    </section>
  );
}
